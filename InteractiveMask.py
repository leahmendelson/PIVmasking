...
# cameras
# camIDs = ['BCL','BCR','BL','BR','TC','TL','TR']
camIDs = ['BL','BR']
ncams = len(camIDs)

start_mask = 360# first image to segment
last_mask = 530 # last image that needs masking
override = 0 # whether to override old images
mask = None # pre-initialize mask

# set up paths

datapath = r"C:\Users\leahr\Dropbox (Personal)\Scholarship\ArcherFish\3D kinematics\061416\fish1jump1"
outpath = r"C:\Users\leahr\Dropbox (Personal)\Scholarship\ArcherFish\3D kinematics\061416\fish1jump1\bin"
...
if __name__ == "__main__":    
    ...
    for i in range(0,ncams):
        ...
        all_imgs = sorted(glob.glob(datapath+'/'+camIDs[i]+'/*.tif'))
        use_imgs = all_imgs[start_mask:last_mask]
        num_imgs = len(use_imgs)
    
        # Check to make sure images were found
        if num_imgs == 0:
            print('No images found. Exiting. Check the path.')
            exit()
        folder = camIDs[i]
    
        savepath = outpath+folder
